...
async function LoginUser(loginData) {
  try {
    const resp=await axios.post('https://healthcompanion.onrender.com:5000/api/auth/login',loginData)
    console.log(resp)
    await  swal.fire({
      icon:"success",
...
async function DataOnPageLoad(token,role) {
  try {
    let resp=''
    if(role==0){
      resp=await axios.get("https://healthcompanion.onrender.com:5000/api/auth/loadData/patient",{
        headers:{
          'authorization':`${token}`
        }
      })
      ...
      resp=await axios.get("https://healthcompanion.onrender.com:5000/api/auth/loadData/doctor",{
        headers:{
          'authorization':`${token}`
        }
      })
      ...
      resp=await axios.get("https://healthcompanion.onrender.com:5000/api/auth/loadData/admin",{
        headers:{
          'authorization':`${token}`
        }
      })
...
async function CheckUserPresent(data) {
  try {
    console.log(data)
    const resp=await axios.post('https://healthcompanion.onrender.com:5000/api/auth/checkUserPresent',data)
    console.log(resp)
...
export async function GoogleSignIN(token) {
  try {
    console.log(token)
    // const resp=null
    const resp=await axios.post('https://healthcompanion.onrender.com:5000/api/auth/google/checkUserPresent',{token: token})
    if(!resp.data.userData){
      ...
      const resp2=await axios.post("https://healthcompanion.onrender.com:5000/api/user/google/createUser",{token:token,role:role.value})
